"""Resumes TimeThenSpaceModel training from its existing best checkpoint
(epoch 2, val_mae=3.077, deliberately stopped early during the original
CPU/deadline-constrained run while still improving -- see report Section 3)
rather than restarting from scratch. Extends the epoch ceiling now that this
project has moved to GPU hardware, letting TTS train for real instead of
being cut off for CPU time-budget reasons.

Run: .venv/Scripts/python.exe -m code.experiments.resume_tts
"""
...
def main():
    ...
    ckpt_path = find_best_checkpoint("tts")
    log.milestone(
        f"=== Q1 TTS: RESUMING from {ckpt_path} (epoch {epoch_of(ckpt_path)}, "
    ...
    status_mod.set_stage("tts_eval", "done", notes=f"60min MAE={overall['60min']['mae']:.3f} (resumed run, GPU)")

    regenerate_outstanding()
    log.milestone(f"=== Q1 TTS: RESUME COMPLETE. Summary: {summary} ===")
    print("\nDone. See results/tts/metrics.json, training_history.json")
